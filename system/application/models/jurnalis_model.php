<?php
class Jurnalis_model extends Model
	{
		function Jurnalis_model()
		{
			parent::Model();
		}
				...
				function Status_App($tabel)
		{
			...
			return $q;
				...
				function Update_Status_app($tabel,$isi,$seleksi)
		{
		...
		function Berita($offset,$limit, $user)
		{
			$q=$this->db->query("select id_berita, judul_berita, isi, gambar, tanggal, waktu, author, counter, status_app,wilayah as kawasan  from tbl_berita inner  join tbl_wilayah on tbl_berita.kawasan   = tbl_wilayah.id_wialayah where author = '".$user."' order by id_berita DESC LIMIT $offset,$limit");
			return $q;
		}
		function Komunitas($offset,$limit)
		{
			$q=$this->db->query("select * from tbl_komunitas inner  join tbl_wilayah on tbl_komunitas.wilayah  = tbl_wilayah.id_wialayah  order by id_komunitas DESC LIMIT $offset,$limit");
			return $q;
		}
		function Total_Artikel($tabel,$user)
		{
			$q=$this->db->query("select * from $tabel where author = '".$user."'");
			return $q;
		}
		...
		function wilayah($offset,$limit)
		{
			$q=$this->db->query("select * from tbl_wilayah order by id_wialayah DESC LIMIT $offset,$limit");
			return $q;
		}
		...
		function Tampil_Data_Wilayah()
		{
			$q=$this->db->query(" select * from tbl_wilayah");
			return $q;
		}
		function getJenis(){
		...
		function Kreasipuisi($offset,$limit,$user)
		{
			$q=$this->db->query("select * from tbl_kreasi where category_kreasi ='1' and  author = '".$user."' order by id_kreasi  DESC LIMIT $offset,$limit");
			return $q;
		}
				function BeriSta($offset,$limit, $user)
		{
			$q=$this->db->query("select id_berita, judul_berita, isi, gambar, tanggal, waktu, author, counter, status_app,wilayah as kawasan  from tbl_berita inner  join tbl_wilayah on tbl_berita.kawasan   = tbl_wilayah.id_wialayah where author = '".$user."' order by id_berita DESC LIMIT $offset,$limit");
			return $q;
		}
		function Total_Kreasipuisi($tabel, $user)
		{
			$q=$this->db->query("select * from $tabel where author = '".$user."'");
			return $q;
		}
		function Kreasicerpen($offset,$limit, $user)
		{
			$q=$this->db->query("select * from tbl_kreasi where category_kreasi ='2' and   author = '".$user."' order by id_kreasi DESC LIMIT $offset,$limit");
			return $q;
		}
		function Total_Kreasicerpen($tabel, $user)
		{
			$q=$this->db->query("select * from $tabel where category_kreasi ='2' and   author = '".$user."'");
			return $q;
		}
	function Kreasihoror($offset,$limit, $user)
		{
			$q=$this->db->query("select * from tbl_kreasi where category_kreasi ='3' and   author = '".$user."' order by id_kreasi DESC LIMIT $offset,$limit");
			return $q;
		}
		function Total_Kreasihoror($tabel, $user)
		{
			$q=$this->db->query("select * from $tabel where category_kreasi ='3' and   author = '".$user."'");
			return $q;
		}
		function Total_Kreasipantun($tabel, $user)
		{
			$q=$this->db->query("select * from $tabel where category_kreasi ='4'  and   author = '".$user."'");
			return $q;
		}
		function Kreasipantun($offset,$limit, $user)
		{
			$q=$this->db->query("select * from tbl_kreasi where category_kreasi ='4' and author = '".$user."' order by id_kreasi DESC LIMIT $offset,$limit");
